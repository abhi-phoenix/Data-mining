...
def Task1(sc,case_number,support, input_file):
    ...
    if case_number == 1:
        ...
        sup_thres = support/textRDD3.getNumPartitions()
        #print('here',textRDD3.getNumPartitions() )
        chunk_process = textRDD3.mapPartitions(lambda chunk: proc_partition(chunk,sup_thres))
    else:
        textRDD3 = textRDD2.map(lambda row: (row[1],row[0])).distinct().groupByKey().map(lambda x: (x[0],list(x[1])))
        sup_thres = support/textRDD3.getNumPartitions()
        chunk_process = textRDD3.mapPartitions(lambda big_chunk: proc_partition(big_chunk, sup_thres))
    #if case_number == 1:
    #print('before',chunk_process.collect())
    reduced = chunk_process.reduceByKey(lambda a,b: sorted(set(a+b))) #.map(lambda x: (x[0],list(x[1])))
    #print('bg',reduced.collect())
...
if __name__ == '__main__':
    if len(sys.argv[1:]) == 4:
        ...
        cands_l,freq_l = Task1(sc,case_number,support, input_file)
        #print('phase 2',cands_l,'here',freq_l,'lop')
